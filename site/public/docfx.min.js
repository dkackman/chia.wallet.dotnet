...
`)})}function Cl(c){localStorage.setItem("theme",c),c==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",c)}async function Xl(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Zl(){Cl(await Xl())}function Bl(c){return new MutationObserver(()=>c(Rb())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function Rb(){return document.documentElement.getAttribute("data-bs-theme")}async function Wl(c){let t=await Xl(),i=t==="light"?"sun":t==="dark"?"moon":"circle-half";return u`
    <div class='dropdown'>
      ...
      <ul class='dropdown-menu dropdown-menu-end'>
        ...
        </div>`;return u`${G} ${r()}`}function r(){let d=Math.max(0,Math.min(s-2,n-5)),C=Math.min(n,d+5),G=Array.from(new Array(C-d).keys()).map(Z=>Z+d);if(G.length<=1)return null;return u`
        <nav>
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link ${Ut({disabled:s<=0})}" href="#" aria-label="Previous"
                @click="${()=>B(s-1)}">
      ...
      </form>`;function G(B){I=B.target.value.trim(),localStorage?.setItem("tocFilter",I),X()}}function d(){return I===""?null:u`<div class='no-result'>${L("searchNoResults",{query:I})}</div>`}function C(){return e?u`<div class="py-2 mb-md-4"><a class="pdf-link" href="${new URL(l||"toc.pdf",i)}">${L("downloadPdf")}</a></div>`:null}}function Js(c,t){let i=document.getElementById("nextArticle");if(!i)return;let b=s(c),e=b.findIndex(g=>g===t),l=b[e-1],o=b[e+1];if(!l&&!o)return;let I=l?u`<div class="prev"><span><i class='bi bi-chevron-left'></i> ${L("prevArticle")}</span> <a href="${l.href}" rel="prev">${ct(l.name)}</a></div>`:null,a=o?u`<div class="next"><span>${L("nextArticle")} <i class='bi bi-chevron-right'></i></span> <a href="${o.href}" rel="next">${ct(o.name)}</a></div>`:null;ot(u`${I} ${a}`,i);function s(g){let n=[];for(let X of g)X.href&&n.push(X),X.items&&n.push(...s(X.items));return n}}var Ul=Nl(Rl(),1);async function hl(){let c=document.getElementById("navbar");if(!c)return[];let{iconLinks:t}=await D(),i=await I(),b=Ns(i),e=a=>u`<li class='nav-item'><a class='nav-link ${a===b?"active":null}' aria-current=${a===b?"page":!1} href=${a.href}>${ct(a.name)}</a></li>`,l=u`
    <ul class='navbar-nav'>${i.map(a=>{if("items"in a){let s=a.items.some(g=>g===b)?"active":null;return u`
      ...
      <form class="icons">
        ...
        </ol>`,t)}async function Al(){await Ls();let c=document.getElementById("affix");c&&ot(ws(),c)}async function Ls(){let c=new Ul.default,{anchors:t}=await D();c.options=Object.assign({visible:"hover",icon:"#"},t),c.add("article h2:not(.no-anchor), article h3:not(.no-anchor), article h4:not(.no-anchor)"),location.hash&&(location.href=location.href)}function ws(){let c=Array.from(document.querySelectorAll("article h2, article h3"));if(c.length>0)return u`
      <h5 class="border-bottom">${L("inThisArticle")}</h5>
      <ul>${c.map(t=>t.tagName==="H2"?u`<li><a class="link-body-emphasis" href="#${t.id}">${ct(t.textContent)}</a></li>`:u`<li><a class="link-secondary" href="#${t.id}">${ct(t.textContent)}</a></li>`)}</ul>`}function Ns(c){let t=new URL(window.location.href),i,b=0;for(let e of c.map(l=>"items"in l?l.items:l).flat()){if(pc(e.href))continue;let l=ks(t,e.href);l===b?i=void 0:l>b&&(b=l,i=e)}return i}function ks(c,t){let i=c.pathname.split("/"),b=t.pathname.split("/"),e=0;for(;e<i.length&&e<b.length&&i[e]===b[e];)e++;return e}async function js(){window.docfx=window.docfx||{};let{start:c}=await D();c?.(),navigator.userAgent.indexOf("docfx/pdf")>=0?await Promise.all([hb(),Qb()]):await Promise.all([Zl(),ul(),Al(),hb(),i(),Qb()]),window.docfx.ready=!0;async function i(){let[b,e]=await Promise.all([hl(),Ql()]);Vl([...b,...e])}}js().catch(console.error);
